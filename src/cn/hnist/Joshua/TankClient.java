...
{
	public static final int  WIN_WIDTH=800;	//静态不可改变的宽度800
	public static final int  WIN_HEIGHT=600;	//静态不可改变的高度600
	boolean repaintFlag=true;  //控制重画线程的结束  标志位
	Image bkImage=null;  //用于双缓冲的缓存图片
	Point mousePoint=new Point(600,500);  //像素区域内
	...
	void lunchFrame()  //界面初始化
	{
		...
		//this.wallList.add(new Wall(200,200,2,25,this));  //添加障碍物ddddd
		...
		this.setLocation(200,100);  //设置区域
		this.setSize(WIN_WIDTH,WIN_HEIGHT);  //设置大小
		this.setTitle("TankWar");
		this.setResizable(false); //不能改变窗口大小
		//this.setBackground(Color.black); //改变颜色
		this.setBackground(Color.gray); //改变颜色

		this.addWindowListener(new FrameClose());  //关闭窗口的监听
		this.addKeyListener(new TankMoveLis()); //玩家Tank按键监听者
		this.addMouseMotionListener(new MouseMoveLis()); //鼠标移动监听者
	...
	class FrameClose extends WindowAdapter  //窗口关闭事件监听者  窗口的适配器
	{
	...
	public void paint(Graphics g)  //synchronized  重绘的画笔
	{
	...
	{
		public void mouseMoved(MouseEvent e)  //MouseEvent 鼠标事件
		{
			if(ut.isLive)  //Tank是否存活 isLive
			{
