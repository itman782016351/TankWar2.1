...
abstract class Tank
{
	...
	TLK.getImage(Tank.class.getClassLoader().getResource("Image/Tank/turret1.png")),
	TLK.getImage(Tank.class.getClassLoader().getResource("Image/Tank/turret2.png"))};
	...
	{
		TLK.getImage(Tank.class.getClassLoader().getResource("Image/Tank/bodyu1.PNG")),
		TLK.getImage(Tank.class.getClassLoader().getResource("Image/Tank/bodyd1.PNG")),
		TLK.getImage(Tank.class.getClassLoader().getResource("Image/Tank/bodyl1.PNG")),
		TLK.getImage(Tank.class.getClassLoader().getResource("Image/Tank/bodyr1.PNG"))
	};
	public static final Image[] TANKBODY2=
	{
		TLK.getImage(Tank.class.getClassLoader().getResource("Image/Tank/bodyu2.PNG")),
		TLK.getImage(Tank.class.getClassLoader().getResource("Image/Tank/bodyd2.PNG")),
		TLK.getImage(Tank.class.getClassLoader().getResource("Image/Tank/bodyl2.PNG")),
		TLK.getImage(Tank.class.getClassLoader().getResource("Image/Tank/bodyr2.PNG"))
	};
	...
	{
		int length=(int)Math.hypot(p.x-x,p.y-y);  //自已找下hypot
		if(0==length) { return; }  //若鼠标和Tank中心重合则不改变
		turretDirx=x+TURRETLENGTH*(p.x-x)/length; //TURRETLENGTH=30
		turretDiry=y+TURRETLENGTH*(p.y-y)/length;

		nowAngel=Math.atan2(x-p.x,y-p.y);  //自已找下atan2()方法
		...
		if(oldAngel!=nowAngel)
		{
			...
			if(this.camp) { g2.drawImage(turretImg[0],null,null); }
			else { g2.drawImage(turretImg[1],null,null); }
		}
	...
	public void draw(Graphics g)
	{
		...
		if(isLive)
		{
			if(this.camp)
			{
				...
				{
					case U:
					case D:{ g.drawImage(imgMap1.get(tankDir),x-18,y-19,null); }break;
					case L:
					case R:{ g.drawImage(imgMap1.get(tankDir),x-19,y-18,null); }break;
					default:{}break;
				}
			}
			else
			{
				...
				{
					case U:
					case D:{ g.drawImage(imgMap2.get(tankDir),x-18,y-19,null); }break;
					case L:
					case R:{ g.drawImage(imgMap2.get(tankDir),x-19,y-18,null); }break;
					default:{}break;
				}
			}
		...
		{
			if(tankBomb!=null) tankBomb.draw(g);
		}
	...
	{	
		if(this.isLive&&moveDir!=TANK_DIR.STOP)
		{
			...
			if(this instanceof UserTank) { setTurretDir(tc.mousePoint); } //玩家Tank炮筒随鼠标转动
			else { setTurretDir(tankDir); }
...
{
	public static enum RTANKTYPE {NOR,SPE,BOSS}; //机器人Tank的类型
	public static ArrayList<TANK_DIR> HITDIR=new ArrayList<TANK_DIR>(); //处理碰撞时用到
	static{ HITDIR.add(TANK_DIR.U);HITDIR.add(TANK_DIR.D);HITDIR.add(TANK_DIR.L);
	HITDIR.add(TANK_DIR.R);HITDIR.add(TANK_DIR.STOP);}
	...
	{
		if(count>20) { count=20; } //一次添加的数目不能多于20
		...
		for(int i=0;i<count;)
		{
			...
			switch(type)
			{
				...
				default:{tempTank=new BossRobotTank(randx,randy,camp,30,tc);}break;
			}
	...
	public void hitDispose()  //处理碰撞
	{
		...
		if(hitFlag)  //若发生碰撞,则从新选择一个新方向
		{
			...
			if(hitDir.size()>0) { setMoveDir(hitDir.get(Tank.rn.nextInt(hitDir.size()))); }
		}
	}
	...
	public void autoAction()
	{
		...
		{
			if(x+26>TankClient.WIN_WIDTH) {	notMoveDir1=0; }
			else if(x-30<0) { notMoveDir1=1; }
			if(y+25>TankClient.WIN_HEIGHT) { notMoveDir2=2; }
			else if(y-50<0) { notMoveDir2=3; }
		}
		...
		if(notMoveDir1!=-1||notMoveDir2!=-1)
		{
			...
			switch(change)
			{
				case 3:{ setMoveDir(TANK_DIR.U); }break;
				case 2:{ setMoveDir(TANK_DIR.D); }break;
				case 1:{ setMoveDir(TANK_DIR.L); }break;
				case 0:{ setMoveDir(TANK_DIR.R); }break;
				default: break;
			}
		}
		else
		{
			...
			switch(change)
			{
				case 7:
				case 6:{ fire(); }break;
				case 5:{ setMoveDir(Tank.TANK_DIR.U); }break;
				case 4:{ setMoveDir(Tank.TANK_DIR.D); }break;
				case 3:{ setMoveDir(Tank.TANK_DIR.L); }break;
				case 2:{ setMoveDir(Tank.TANK_DIR.R); }break;
				case 1:{ setMoveDir(Tank.TANK_DIR.STOP); }break;
				default: break;
			}
		}
...
class BrainRobotTank extends RobotTank  //智能较高的机器人Tank
{
	...
	public void autoAction()
	{
		if(0==Tank.rn.nextInt(50)) { attackOtherCamp(); }
		else { super.autoAction(); }
	}
...
System.out.println("中弹,赶快躲避");
	...
	public void attackOtherCamp()  //攻击敌对阵营Tank
	{
		...
		for(int i=0;i<tc.tanksList.size();i++)
		{
			...
			{
				attackCount++;
				if(2==attackCount) { break; }  //一轮攻击共发射3枚炮弹,避免发射太多
				...
				if(otk.moveDir!=Tank.TANK_DIR.STOP) //Tank在运动,根据运动方向打提前量
				{
					...
					switch(otk.moveDir)
					{
						case U:{ setTurretDir(new Point(otk.x,otk.y-shift)); }break;
						case D:{ setTurretDir(new Point(otk.x,otk.y+shift)); }break;
						case L:{ setTurretDir(new Point(otk.x-shift,otk.y)); }break;
						case R:{ setTurretDir(new Point(otk.x+shift,otk.y)); }break;
						default:break;
					}
...
class BossRobotTank extends BrainRobotTank   //Boos机器人Tank
{
	...
	public static TANK_DIR[] dirArray={TANK_DIR.U,TANK_DIR.D,TANK_DIR.L,TANK_DIR.R};
	TANK_DIR[] toMoveWay={null,null,null,null};//躲避时的移动路径
	...
	public void autoAction()
	{
		if(!avoidAttack)
		{
			if(Tank.rn.nextInt(16)==0) { scanThrust(); } //定时扫描
			else { super.autoAction(); }
		}
		else
		{
			if(avoidStep>=0)
			{
				...
				this.move();
				avoidStep--;
				if(avoidStep==-1) { avoidAttack=false;}
			}
		}
	}
...
System.out.println("敌弹来袭!!!\n"+"子弹数:"+scopeShots.size());
		...
		{ notMoveDir1=0; notMoveDir2=1; }
		else if(Math.abs((double)issDir.dirY)/(Math.abs(issDir.dirX)+0.001)<0.3) //水平方向
		{ notMoveDir1=2; notMoveDir2=3; }
		else { notMoveDir1=-1; notMoveDir2=-1; }
		...
		for(int s1=0;s1<4;s1++)
		{
			...
			if(s1==notMoveDir1||s1==notMoveDir2) { continue; } //不向平行于子弹的方向走
			
			if(s1==0) { tempY-=10; }
			else if(s1==1) { tempY+=10; }
			else if(s1==2) { tempX-=10; }
			else if(s1==3) { tempX+=10; }
			if(!ableHitInScope(new Rectangle(tempX-20,tempY-20,40,40))
				&&tempY>50&&tempY<TankClient.WIN_HEIGHT-25&&tempX>25&&x<TankClient.WIN_WIDTH-25)
			{
				TANK_DIR[] tempDir={dirArray[s1]};
				toMoveWays.add(tempDir);
				if(toMoveWays.size()>WAYSCOUNT)  //只存储WAYSCOUNT路径 
				{ choiceMoveDir(); return; }
			}
			for(int s2=0;s2<4;s2++)
			{
				if(s2==notMoveDir1||s2==notMoveDir2) { continue; } //不向平行于子弹的方向走
				
				if(s2==0) { tempY-=10; }
				else if(s2==1) { tempY+=10; }
				else if(s2==2) { tempX-=10; }
				else if(s2==3) { tempX+=10; }
				if(!ableHitInScope(new Rectangle(tempX-20,tempY-20,40,40))
					&&tempY>50&&tempY<TankClient.WIN_HEIGHT-25&&tempX>25&&x<TankClient.WIN_WIDTH-25)
				{
					TANK_DIR[] tempDir={dirArray[s2],dirArray[s1]};
					toMoveWays.add(tempDir);
					if(toMoveWays.size()>WAYSCOUNT) { choiceMoveDir(); return; }
				}
				for(int s3=0;s3<4;s3++)
				{
					if(s3==0) { tempY-=10; }
					else if(s3==1) { tempY+=10; }
					else if(s3==2) { tempX-=10; }
					else if(s3==3) { tempX+=10; }
					if(!ableHitInScope(new Rectangle(tempX-20,tempY-20,40,40))
						&&tempY>50&&tempY<TankClient.WIN_HEIGHT-25&&tempX>25&&x<TankClient.WIN_WIDTH-25)
					{
						TANK_DIR[] tempDir={dirArray[s3],dirArray[s2],dirArray[s1]};
						toMoveWays.add(tempDir);
						if(toMoveWays.size()>WAYSCOUNT) { choiceMoveDir(); return; }
					}
					...
					{
						if(s4==0) { tempY-=10; }
						else if(s4==1) { tempY+=10; }
						else if(s4==2) { tempX-=10; }
						else if(s4==3) { tempX+=10; }
						if(!ableHitInScope(new Rectangle(tempX-20,tempY-20,40,40))
							&&tempY>50&&tempY<TankClient.WIN_HEIGHT-25&&tempX>25&&x<TankClient.WIN_WIDTH-25)
						{
							TANK_DIR[] tempDir={dirArray[s4],dirArray[s3],dirArray[s2],dirArray[s1]};
							toMoveWays.add(tempDir);
							if(toMoveWays.size()>WAYSCOUNT) { choiceMoveDir(); return; }
						}
					}
		...
		if(toMoveWays.size()>0) { choiceMoveDir(); } //有可以躲避的移动方向
		this.move();
	}
			...
			for(int j=tempWay.length-1;j>=0;j--)  //计算按此tempDir移动到达的最远点
			{
				switch(tempWay[j])
				{
					...
					{ 
						...
						{ endy-=10; } }break;
					case D:{ if(Math.abs(endy+10-y)>Math.abs(endy-y)) { endy+=10; } }break;
					case L:{ if(Math.abs(endx-10-x)>Math.abs(endx-x)) { endx-=10; } }break;
					case R:{ if(Math.abs(endx+10-x)>Math.abs(endx-x)) { endx+=10; } }break;
					default:break;
				}
			}
...
class UserTank extends Tank  //由玩家自主控制的Tank
{
	...
	static final Image SHOTS[]={
		TLK.getImage(Tank.class.getClassLoader().getResource("Image/Tank/ammo1.png")),
		TLK.getImage(Tank.class.getClassLoader().getResource("Image/Tank/ammo2.png"))
	};
	...
	public void keyDispose(int Key,boolean b)  //对用户按键的处理
	{
		...
		if(keyList.size()==0) { keyList.add(TANK_DIR.STOP); }
